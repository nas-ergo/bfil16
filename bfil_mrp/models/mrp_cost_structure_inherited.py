        ...
        for product in productions.mapped('product_id'):
            ...
            if Workorders:
                query_str = """SELECT
                                    ...
                                    currency_table.rate,
                                    wo.wo_cost_per_min as cost_minute
                ...
                for mo_id, dummy_wo_id, op_id, wo_name, wc_name, duration, cost_hour, currency_rate, cost_minute in self.env.cr.fetchall():
                    cost = duration * cost_minute * currency_rate
                    total_cost_by_mo[mo_id] += cost
                    operation_cost_by_mo[mo_id] += cost
                    total_cost_operations += cost
                    operations.append([wc_name, op_id, wo_name, duration, cost_minute * currency_rate])
